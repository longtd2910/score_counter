...
class GameProvider with ChangeNotifier {
  ...
  Future<String?> saveCurrentGame() async {
    ...
    if (_players.isNotEmpty && _currentGameMode != null) {
      savedGameId = await _storageService.saveGameSnapshot(_currentGameMode!, _players);
    ...
    switch (lastEntry.actionType) {
      case ActionType.scoreChanged:
        ...
        final playerIndex = _players.indexWhere(
          (p) => p.name == lastEntry.playerName && p.color == lastEntry.playerColor
        );
  ...
  Future<void> loadSavedGame(String id) async {
    ...
    if (savedGame != null) {
      ...
      final entry = HistoryEntry(
        ...
        additionalInfo: " loaded saved game from ${_formatDate(savedGame.timestamp)}",
      );
      await _addHistoryEntry(entry);
  ...
  Future<void> startNewGame() async {
    ...
    final entry = HistoryEntry(
      ...
      playerName: "System",
      playerColor: Colors.grey,
      additionalInfo: " started new game with game mode: ${_currentGameMode!.title["en"]}",
    );
    await _addHistoryEntry(entry);
